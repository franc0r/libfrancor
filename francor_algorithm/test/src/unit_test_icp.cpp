...
TEST(Icp, EstimateTransformFullAssignment)
{
  ...
  for (Angle angle = Angle::createFromDegree(-29.0); angle < Angle::createFromDegree(30.0); angle += Angle::createFromDegree(1.0))
  {
    const Transform2d transform( { angle }, { 0.3, 0.1 } );
    ...
    ASSERT_TRUE(icp.estimateTransform(origin, target, result)) << "estimate transform fails with " << angle;
